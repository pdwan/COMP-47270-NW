...
LABORATORY | CASE STUDY 2  : algorithimeval.py
    
"""

import networkx as nx							# import the networkx network analysis package
from networkx import graphviz_layout			# import the graphvisualisation package graphviz
import pygraphviz
import matplotlib.pyplot as plt					# import the plotting functionality from matplotlib
from scipy.spatial import Delaunay				#import Delaunay tesselation 
from scipy.cluster.vq import vq, kmeans, whiten	# import kmeans
...
def placement():
    ...
    eigen_pos = dict()												#   eigen_pos holds the positions
    deg = A.sum(0)
    diags = np.array([0])
    D = sp.sparse.spdiags(deg,diags,A.shape[0],A.shape[1])  		# diagonal matrix of degrees
    Dinv = sp.sparse.spdiags(1/deg,diags,A.shape[0],A.shape[1])	# inverse of Normalised laplacian : multiply by 1 / Deg previously 
    L = Dinv*(D - A)
